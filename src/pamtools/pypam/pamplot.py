...
def set_ticksStyle(x_size=4, y_size=4, x_dir="in", y_dir="in"):
    ...
    sns.set_style(
        "ticks",
        {"xtick.major.size": x_size, "ytick.major.size": y_size, "xtick.direction": x_dir, "ytick.direction": y_dir},
    )
...
class Hist2d:
    ...
    def plot2Dhist(
        self,
        style="contour",
        axis_labels=("FRET", "Stoichiometry"),
        cmap="RdGy_r",
        imgOffset=0,
        PAM_fit=True,
        PAMtools_fit=False,
        ...
        with sns.axes_style("ticks"):
            ...
            f, ax = plt.subplots(
                ...
                sharey=False,
                squeeze=False,
                gridspec_kw={"width_ratios": [1, 0.3], "height_ratios": [0.3, 1]},
            )
            ...
            if style == "image":
                ...
                ax[1, 0].imshow(
                    ...
                    cmap=cmap,
                    origin="lower",
                    extent=[min(self.image["x"]), max(self.image["x"]), min(self.image["y"]), max(self.image["y"])],
                )
            elif style == "hex":
                ...
                ax[1, 0].contourf(
                    self.contour["x"], self.contour["y"], self.contour["z"], levels=self.contour["levels"], cmap=cmap
                )
            ax[0, 0].bar(
                self.XY1D[("X", "x")],
                self.XY1D[("X", "y")],
                width=self.XY1D[("X", "x")][1] - self.XY1D[("X", "x")][0],
                color=hist_color,
                edgecolor=edgecolor,
                linewidth=linewidth,
            )
            ax[1, 1].barh(
                self.XY1D[("Y", "x")],
                self.XY1D[("Y", "y")],
                height=self.XY1D[("Y", "x")][1] - self.XY1D[("Y", "x")][0],
                color=hist_color,
                edgecolor=edgecolor,
                linewidth=linewidth,
            )
            ...
            if PAM_fit:
                for i in range(self.XY1DfitComp[("X", "x")].shape[0]):
                    ax[0, 0].plot(
                        self.XY1DfitComp[("X", "x")][i, :],
                        self.XY1DfitComp[("X", "y")][i, :],
                        color=colors[i],
                        linestyle="--",
                    )
                    ax[1, 1].plot(
                        self.XY1DfitComp[("Y", "y")][i, :],
                        self.XY1DfitComp[("Y", "x")][i, :],
                        color=colors[i],
                        linestyle="--",
                    )
            ...
            if label:
                ax[1, 0].text(
                    self.limits["x"][0] + np.diff(self.limits["x"]) / 10,
                    self.limits["y"][1] - np.diff(self.limits["y"]) / 7,
                    label,
            ...
            if PAMtools_fit and "X" in self.XY1DfitParam_PAMtools:
                ...
                if n > 1 and show_components:
                    for i in range(n):
                        ax[0, 0].plot(
                            self.XY1DfitSum_PAMtools[("X", "x")],
                            self.XY1DfitComp_PAMtools[("X", "y", i)],
                            ":",
                            color=colors[i],
                            linewidth=1.5,
                        )
            ...
            if PAMtools_fit and "Y" in self.XY1DfitParam_PAMtools:
                ...
                if m > 1 and show_components:
                    for i in range(m):
                        ax[1, 1].plot(
                            self.XY1DfitComp_PAMtools[("Y", "y", i)],
                            self.XY1DfitSum_PAMtools[("Y", "x")],
                            ":",
                            color=colors[i],
                            linewidth=1.5,
                        )
    ...
    def plot_BVA(
        ...
        self.plot2Dhist(
            style=style,
            axis_labels=(x_axis_label, "BVA st.dev."),
            cmap=cmap,
            imgOffset=imgOffset,
            PAM_fit=PAM_fit,
            PAMtools_fit=PAMtools_fit,
            label=label,
    ...
    def beta(x_data, mu, sigma):
        ...
        beta = (
            scipy.special.gamma(A + D)
            / (scipy.special.gamma(A) * scipy.special.gamma(D))
            * x_data ** (A - 1)
            * (1 - x_data) ** (D - 1)
        )
        return beta
    ...
    def PR2FRET(self, proximity_ratio, verbose=False):
        ...
        try:
            FRET = (
                1 - (1 + self.parameters["crosstalk"] + self.parameters["direct_excitation"]) * (1 - proximity_ratio)
            ) / (1 - (1 + self.parameters["crosstalk"] - self.parameters["gamma_factor"]) * (1 - proximity_ratio))
        except KeyError:
            FRET = None
            if verbose:
    ...
    def sigma_shotnoise(self, verbose=False):
        ...
        try:
            ...
            if verbose:
                print(
                    'Parameter "photons_per_window" is missing. Expected proximity ratio variance can not be calculated.'
                )
...
class FCS:
    ...
    def plotFCS(
        self,
        color=ncp.get_colors(bwo, 1),
        time_limits=(10**-7, 0.1),
        legend_strings=None,
        normalization="None",
        normalization_time=10**-6,
        linecolor="k",
        figsize=(2.25, 2),
    ):
        """
...
class Timetrace:
    ...
    def plotTrace(self, time_limits=[0, 1], count_limits=[-50, 50], figsize=(5, 2)):
        ...
        with sns.axes_style("ticks"):
            ...
            ax[0, 0].plot(
                self.GG.loc[(self.GG["time"] > time_limits[0]) & (self.GG["time"] < time_limits[1]), "time"],
                self.GG.loc[(self.GG["time"] > time_limits[0]) & (self.GG["time"] < time_limits[1]), "counts"],
                color=colors[0],
            )
            ax[0, 0].plot(
                self.GR.loc[(self.GR["time"] > time_limits[0]) & (self.GR["time"] < time_limits[1]), "time"],
                -self.GR.loc[(self.GR["time"] > time_limits[0]) & (self.GR["time"] < time_limits[1]), "counts"],
                color=colors[1],
            )
